...
> Cập nhật **2026-08-15** · Lane sở hữu: **NON-CODE** · Khuôn: [task_guiline.md](../task_guiline.md) · Kế hoạch gốc: [prompt-fullstack.md](../prompt-fullstack.md)
...
`finding_project_preparation.md` (nhà của nó là **thư mục chứa file này**, không phải gốc repo) — mọi thứ đang sai hôm nay đều đã có mã task nhận (link gãy → T-03 · bản đồ thiếu trọng tài → T-14 · số trần trụi → T-15), chưa có dòng nào *"đang sai mà không ai nhận"*.
...
Cột **Chặn bởi** — mở cùng lúc với sổ finding trên. Khi mở: finding task này **đóng** thì ghi ở cột *Đầu ra*, **không** ghi ở cột *Chặn bởi* (đây là chỗ đời trước giết 8 task).
...
| finding đang MỞ | > 20 dòng | (chưa có `finding_project_preparation.md`) |
